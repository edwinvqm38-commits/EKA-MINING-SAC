...
if (typeof PERMISSIONS_SHEET_NAME === 'undefined') {
  var PERMISSIONS_SHEET_NAME = 'Permisos Invitaciones';
}

if (typeof assertUserCanEditInvitations !== 'function') {
  var assertUserCanEditInvitations = function () {
    var permissions = getCurrentUserPermissions();
    if (!permissions || permissions.canEdit === false) {
      throw new Error(
        'No tienes permisos para editar invitaciones. Verifica tu rol en la hoja "' +
...
if (typeof assertUserCanSyncInvitations !== 'function') {
  var assertUserCanSyncInvitations = function () {
    var permissions = getCurrentUserPermissions();
    if (!permissions || permissions.canSync === false) {
      throw new Error(
        'No tienes permisos para sincronizar con Supabase. Revisa la configuración en "' +
          PERMISSIONS_SHEET_NAME +
          '".'
      );
    }
...
if (typeof assertUserCanDeleteInvitations !== 'function') {
  var assertUserCanDeleteInvitations = function () {
    var permissions = getCurrentUserPermissions();
    if (!permissions || permissions.canDelete === false) {
      throw new Error(
...
const FIELD_LOOKUP = getInvitationFieldBlueprint();
const COLUMN_MAP = getInvitationColumnMap();
const CONTACT_FIELD_MAP = (function () {
  const map = {};
  getContactRoles().forEach(function (role) {
    map[role.key] = [role.email, role.phone];
  });
  return map;
})();
...
function getInvitationSheetConfig() {
  return {
    ...
    fieldSections: getInvitationFieldSections(),
  };
}
...
function showSidebar() {
  ...
  const sections = config.fieldSections;
...
function buildContactDirectory() {
  const directory = {};
  Object.keys(CONTACT_FIELD_MAP).forEach(function (role) {
    directory[role] = {};
  });
  const sheet = getInvitationSheet();
  const lastRow = sheet.getLastRow();
  ...
  const indexMap = {};
  Object.keys(CONTACT_FIELD_MAP).forEach(function (role) {
    const pair = CONTACT_FIELD_MAP[role];
    indexMap[role] = {
      name: headers.indexOf(COLUMN_MAP[role]),
      email: headers.indexOf(COLUMN_MAP[pair[0]]),
      phone: headers.indexOf(COLUMN_MAP[pair[1]]),
    };
  });
  ...
  values.forEach(function (row) {
    Object.keys(CONTACT_FIELD_MAP).forEach(function (role) {
      const indexes = indexMap[role];
      const nameIndex = indexes.name;
      if (nameIndex === -1) {
        return;
      }
...
function clearAssociatedContacts(alias, rowIndexes) {
  ...
  const relatedAliases = CONTACT_FIELD_MAP[alias];
  if (!relatedAliases || !relatedAliases.length) {
    return;
  }
  const headers = getHeaders();
  const sheet = getInvitationSheet();
